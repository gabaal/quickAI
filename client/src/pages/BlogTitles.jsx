...
const BlogTitles = () => {
  ...
  const blogCategories = [
      'Gereral', 'Technology', 'Health', 'Lifestyle', 'Travel', 'Education', 'Finance', 'Entertainment', 'Food', 'Sports'
    ]
    const [selectedCategory, setSelectedCatgegory] = useState('Gereral')
...
const { getToken } = useAuth()
    ...
    const onSubmitHandler = async (e) => {
      ...
      try {
        setLoading(true)
        const prompt = `Generate some catchy titles for a blog post about ${input} in the category ${selectedCategory}`
  ...
  return (
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        ...
        <p className="mt-6 text-xm font-medium">Keyword</p>
        <input onChange={(e) => setInput(e.target.value)} value={input} type='text' className="w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300" placeholder='The future of AI..' required />
        <p className="mt-4 text-sm font-medium">Category</p>
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {blogCategories.map((item) => (
            <span onClick={() => setSelectedCatgegory(item)}
...
{!content ? ( <div className="flex-1 flex justify-center items-center">
          <div className="text-sm flex flex-col items-center gap-5 text-gray-400">
            ...
            <p className="">Enter keyword and click 'Generate Title' to get started</p>
          </div>
